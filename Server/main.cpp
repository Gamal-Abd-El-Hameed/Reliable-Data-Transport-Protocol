...
int main()
{
    ...
    /// the first line contains port number
    ...
    /// save memory for server server and client addresses
    ...
    /// the  next code is for client connection
    while (true){
        ...
        /// check if the received bytes are valid
        if (Received_bytes <= 0){
        ...
        /// error while creating the process
         if (pid == -1){
             ...
             /// create socket for client request
...
/// read related commands from the file (e.g. port number)
vector<string> readCommand(){
...
void handle_client_request(int server_socket, int client_fd, struct sockaddr_in client_addr, char rec_buffer [] , int bufferSize) {
     ...
     /// check the existence of the file in the request
